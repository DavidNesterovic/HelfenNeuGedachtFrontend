<template>
  <transition name="fade">
    ...
    >
      <div class="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-gray-100 flex flex-col transform transition-all duration-300 scale-100">
        <!-- Header -->
        <div class="px-5 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
          <h3 class="text-sm font-bold text-gray-800 uppercase tracking-wider">Organisation</h3>
        ...
        <div class="p-6">
          ...
          <div v-if="loading" class="flex flex-col items-center py-12 space-y-3">
            ...
            <span class="text-sm text-gray-500 font-semibold">Lade Organisationsdaten...</span>
          </div>
          ...
          <div v-else-if="error" class="text-center py-8 text-red-500 font-semibold space-y-2">
            ...
            <button @click="closePopup" class="mt-4 bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-xs font-bold transition">
              Schließen
            </button>
          </div>
          ...
          <div v-else-if="orgData" class="space-y-6">
            ...
            <div class="border-b border-gray-100 pb-4">
              <div class="flex items-start justify-between gap-2">
                <h4 class="text-xl font-bold text-gray-900 leading-snug">{{ orgData.name }}</h4>
              ...
              <p v-if="orgData.description" class="mt-3 text-sm text-gray-600 leading-relaxed bg-gray-50 border border-gray-100 p-3.5 rounded-xl">
                {{ orgData.description }}
              </p>
            </div>
            ...
            <div class="space-y-4 text-sm text-gray-700">
              ...
              <div class="flex items-start gap-3">
                ...
                <div>
                  <span class="block font-bold text-xs text-gray-400 uppercase tracking-wide">Adresse</span>
                  <span>{{ orgData.street }}</span>
                  <span class="block">{{ orgData.postalCode }} {{ orgData.city }}</span>
                  <span v-if="orgData.state || orgData.country" class="block text-xs text-gray-500">
              ...
              <div v-if="orgData.contactPersonName" class="flex items-start gap-3">
                ...
                <div>
                  <span class="block font-bold text-xs text-gray-400 uppercase tracking-wide">Ansprechpartner</span>
                  <span class="font-semibold">{{ orgData.contactPersonName }}</span>
                  <span v-if="orgData.contactPersonRole" class="text-xs text-gray-500"> ({{ orgData.contactPersonRole }})</span>
                </div>
              </div>
            ...
            <!-- Close Button -->
            <button @click="closePopup" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-xl transition text-sm shadow-sm">
              Schließen
            </button>
          </div>
        </div>
