<template>
  <div>
    ...
    >
      <div class="absolute inset-0 rounded-lg border-2 overflow-hidden flex items-center justify-center bg-slate-50 transition-colors"
        :class="displayUrl
          ? 'border-slate-200 group-hover:border-blue-400'
          : 'border-dashed border-slate-200 group-hover:border-blue-400'"
      >
        <img v-if="displayUrl" :src="displayUrl" class="w-full h-full object-cover" />
        <div v-else class="flex flex-col items-center gap-2 text-slate-400 group-hover:text-blue-500 transition-colors pointer-events-none">
        ...
        <div v-if="cropVisible" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col">
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <h3 class="font-bold text-slate-900">Bild zuschneiden</h3>
              <button type="button" class="text-slate-400 hover:text-slate-600 transition" @click="cancelCrop">
            ...
            <div class="bg-slate-950 flex items-center justify-center" style="height: 380px">
              <ClientOnly>
                <component
            ...
            <div class="flex gap-3 px-5 py-4 border-t border-slate-100">
              <AppButton type="button" variant="secondary" class="flex-1" @click="cancelCrop">Abbrechen</AppButton>
              <AppButton type="button" class="flex-1" :disabled="!pendingCanvas" @click="confirmCrop">Übernehmen</AppButton>
            </div>
          </div>
        </div>
      </Transition>
