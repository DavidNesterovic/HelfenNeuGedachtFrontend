  ...
  <main class="p-8">
    ...
    <form v-else @submit.prevent="saveChanges" class="space-y-6 max-w-5xl">
      ...
      <section class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 class="text-xl font-bold mb-6 text-gray-800 border-b border-gray-200 pb-2">Grundlegende Informationen</h3>
        
        <div class="grid grid-cols-1 gap-6">
          <div class="flex flex-col">
            <label class="text-sm font-bold text-gray-700 mb-2">Name der Organisation *</label>
            <input v-model="orgData.name" type="text" class="border border-gray-200 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none transition" required>
          </div>

          <div class="flex flex-col">
            <label class="text-sm font-bold text-gray-700 mb-2">Beschreibung</label>
            <textarea v-model="orgData.description" rows="4" class="border border-gray-200 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none transition"></textarea>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex flex-col">
              <label class="text-sm font-bold text-gray-700 mb-2">Typ</label>
              <select v-model="orgData.type" class="border border-gray-200 rounded-lg p-2.5 bg-white">
            ...
            <div class="flex flex-col">
              ...
              <input v-model="orgData.registrationNumber" type="text" class="border border-gray-200 rounded-lg p-2.5" required>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 class="text-xl font-bold mb-6 text-gray-800 border-b border-gray-200 pb-2">Adresse</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="md:col-span-2 flex flex-col">
            <label class="text-sm font-bold text-gray-700 mb-2">Straße und Hausnummer</label>
            <input v-model="orgData.street" type="text" class="border border-gray-200 rounded-lg p-2.5">
          </div>
          <div class="flex flex-col">
            <label class="text-sm font-bold text-gray-700 mb-2">Postleitzahl</label>
            <input v-model="orgData.postalCode" type="text" class="border border-gray-200 rounded-lg p-2.5">
          </div>
          <div class="flex flex-col">
            <label class="text-sm font-bold text-gray-700 mb-2">Stadt</label>
            <input v-model="orgData.city" type="text" class="border border-gray-200 rounded-lg p-2.5">
          </div>
        </div>
      </section>

      <section class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 class="text-xl font-bold mb-6 text-gray-800 border-b border-gray-200 pb-2">Kontaktinformationen</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="flex flex-col">
            <label class="text-sm font-bold text-gray-700 mb-2">Kontakt-E-Mail *</label>
            <input v-model="orgData.contactEmail" type="email" class="border border-gray-200 rounded-lg p-2.5" required>
          </div>
          <div class="flex flex-col">
            <label class="text-sm font-bold text-gray-700 mb-2">Kontakt-Telefon</label>
            <input v-model="orgData.contactPhone" type="tel" class="border border-gray-200 rounded-lg p-2.5">
          </div>
        </div>
      </section>

      <div class="flex gap-4 sticky bottom-0 bg-gray-50 py-4 border-t border-gray-200">
